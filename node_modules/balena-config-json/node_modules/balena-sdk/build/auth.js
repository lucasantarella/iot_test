...
getAuth = function(deps, opts) {
  var apiUrl, auth, exports, getUserDetails, normalizeAuthError, request, twoFactor, userDetailsCache;
  auth = deps.auth, request = deps.request;
  apiUrl = opts.apiUrl;
  twoFactor = require('./2fa')(deps, opts);
  exports = {};
  normalizeAuthError = function(err) {
  ...
  exports.whoami = function(callback) {
    return getUserDetails().then(function(userDetails) {
      return userDetails.username || void 0;
    }).catchReturn(void 0).asCallback(callback);
  };
  ...
  exports.login = function(credentials, callback) {
    userDetailsCache = null;
    return exports.authenticate(credentials).then(auth.setKey).asCallback(callback);
  };
  ...
  exports.isLoggedIn = function(callback) {
    return getUserDetails()["return"](true).catchReturn(false).asCallback(callback);
  };
  ...
  exports.getToken = function(callback) {
    return auth.getKey().then(function(savedToken) {
      return savedToken;
    })["catch"](function(err) {
      throw normalizeAuthError(err);
    }).asCallback(callback);
  ...
  exports.getUserId = function(callback) {
    return getUserDetails().get('id').asCallback(callback);
  };
  ...
  exports.getEmail = function(callback) {
    return getUserDetails().get('email').asCallback(callback);
  };
