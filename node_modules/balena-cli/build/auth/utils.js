...
exports.getDashboardLoginURL = function(callbackUrl) {
  ...
  return balena.settings.get('dashboardUrl').then(function(dashboardUrl) {
...
exports.loginIfTokenValid = function(token) {
  if ((token == null) || _.isEmpty(token.trim())) {
    return Promise.resolve(false);
  }
  return balena.auth.getToken().catchReturn(void 0).then(function(currentToken) {
    return balena.auth.loginWithToken(token)["return"](token).then(balena.auth.isLoggedIn).tap(function(isLoggedIn) {
      if (isLoggedIn) {
        return;
      }
