...
(function() {
  var BalaenaProgressReporter, DEFAULT_PROGRESS_BAR_STEP_COUNT, Docker, DockerProgress, JSONStream, LEGACY_DOCKER_VERSION, ProgressReporter, ProgressTracker, Promise, _, awaitRegistryStream, isBalenaEngine, legacy, onProgressHandler, renderProgress, semver, tryExtractDigestHash,
  ...
  awaitRegistryStream = function(stream, onProgress) {
    var contentHash;
    contentHash = null;
  ...
  BalaenaProgressReporter = (function(superClass) {
    extend(BalaenaProgressReporter, superClass);

    function BalaenaProgressReporter() {
      return BalaenaProgressReporter.__super__.constructor.apply(this, arguments);
    }

    BalaenaProgressReporter.prototype.pullProgress = Promise.method(function(image, onProgress) {
      var lastPercentage, progressRenderer;
      progressRenderer = this.renderProgress;
      lastPercentage = 0;
      return function(evt) {
        var current, err, id, percentage, ref, ref1, total;
        try {
          id = evt.id;
          if (id !== 'Total') {
            return;
          ...
          err = error;
          return console.warn('Progress error:', (ref1 = err.message) != null ? ref1 : err);
        }
      };
    });

    return BalaenaProgressReporter;
  ...
  exports.DockerProgress = DockerProgress = (function() {
    ...
    DockerProgress.prototype.pull = function(image, onProgress, options, callback) {
      ...
      return this.docker.pull(image, options).then(function(stream) {
        return awaitRegistryStream(stream, onProgress);
      }).nodeify(callback);
    };
