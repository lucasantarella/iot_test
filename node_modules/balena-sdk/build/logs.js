 ...
 */
var AbortController, AbortControllerPonyfill, EventEmitter, Promise, errors, findCallback, getLogs, globalEnv, ndjson, querystring, ref;
...
AbortControllerPonyfill = require('abortcontroller-polyfill/dist/cjs-ponyfill').AbortController;

ref = require('./util'), findCallback = ref.findCallback, globalEnv = ref.globalEnv;

AbortController = (globalEnv != null ? globalEnv.AbortController : void 0) || AbortControllerPonyfill;

getLogs = function(deps, opts) {
  var deviceModel, exports, getLogsFromApi, getLogsUrl, request, subscribeToApiLogs;
  request = deps.request;
  deviceModel = require('./models/device')(deps, opts);
  exports = {};
  getLogsUrl = function(device, options) {
    var query;
    query = querystring.stringify(options);
    return "/device/v2/" + device.uuid + "/logs?" + query;
  };
  getLogsFromApi = function(device, options) {
    if (options == null) {
      options = {};
    }
    return request.send({
  ...
  subscribeToApiLogs = function(device, options) {
    var controller, emitter, parser;
    if (options == null) {
      options = {};
    }
    ...
    parser = ndjson();
    request.stream({
      url: getLogsUrl(device, Object.assign({}, options, {
        stream: 1
      })),
      baseUrl: opts.apiUrl,
      ...
      stream.on('error', function(e) {
        return parser.emit('error', e);
      });
      parser.on('data', function(log) {
        if (!controller.signal.aborted) {
          return emitter.emit('line', log);
        }
      });
  ...
  exports.history = function(uuidOrId, options, callback) {
    ...
    return deviceModel.get(uuidOrId, {
      ...
      return getLogsFromApi(device, options);
    }).asCallback(callback);
  };
  return exports;
};
