 ...
 */
var LOCKED_STATUS_CODE, Promise, assign, errors, filter, findCallback, forEach, generateCurrentServiceDetails, getApplicationModel, getCurrentServiceDetailsPineOptions, isArray, isEmpty, isId, mergePineOptions, noApplicationForKeyResponse, normalizeDeviceOsVersion, notFoundResponse, once, ref, size, treatAsMissingApplication;
...
ref = require('../util'), isId = ref.isId, findCallback = ref.findCallback, getCurrentServiceDetailsPineOptions = ref.getCurrentServiceDetailsPineOptions, generateCurrentServiceDetails = ref.generateCurrentServiceDetails, mergePineOptions = ref.mergePineOptions, notFoundResponse = ref.notFoundResponse, noApplicationForKeyResponse = ref.noApplicationForKeyResponse, treatAsMissingApplication = ref.treatAsMissingApplication, LOCKED_STATUS_CODE = ref.LOCKED_STATUS_CODE;

normalizeDeviceOsVersion = require('../util/device-os-version').normalizeDeviceOsVersion;

getApplicationModel = function(deps, opts) {
  var apiUrl, auth, buildDependentResource, configVarModel, deviceModel, envVarModel, exports, getId, normalizeApplication, pine, releaseModel, request, tagsModel;
  request = deps.request, pine = deps.pine;
  apiUrl = opts.apiUrl;
  auth = require('../auth')(deps, opts);
  deviceModel = once(function() {
    return require('./device')(deps, opts);
  });
  releaseModel = once(function() {
    return require('./release')(deps, opts);
  });
  ...
  tagsModel = buildDependentResource({
    ...
    getResourceId: function(nameOrId) {
  ...
  normalizeApplication = function(application) {
    if (isArray(application.owns__device)) {
      forEach(application.owns__device, function(device) {
  ...
  exports.getAllWithDeviceServiceDetails = function(options, callback) {
    ...
    return exports.getAll(serviceOptions).then(function(apps) {
      apps.forEach(function(app) {
        return app.owns__device = app.owns__device.map(generateCurrentServiceDetails);
      });
      return apps;
    }).asCallback(callback);
  };
  ...
  /**
  	 ...
  	 * });
   */
  exports.getWithDeviceServiceDetails = function(nameOrId, options, callback) {
    var serviceOptions;
    if (options == null) {
      options = {};
    }
    ...
    return exports.get(nameOrId, serviceOptions).then(function(app) {
  ...
  exports.has = function(nameOrId, callback) {
    return exports.get(nameOrId, {
      $select: ['id']
    })["return"](true)["catch"](errors.BalenaApplicationNotFound, function() {
      return false;
    }).asCallback(callback);
  };
  ...
  /**
  	 ...
  	 * });
   */
  exports.create = function(arg, callback) {
    var applicationType, applicationTypePromise, deviceManifestPromise, deviceType, name, parent, parentAppPromise;
    name = arg.name, applicationType = arg.applicationType, deviceType = arg.deviceType, parent = arg.parent;
    callback = findCallback(arguments);
    applicationTypePromise = !applicationType ? Promise.resolve() : pine.get({
    ...
    }).get(0).then(function(appType) {
      if (!appType) {
        throw new Error("Invalid application type: " + applicationType);
      }
      return appType.id;
    });
    parentAppPromise = parent ? exports.get(parent, {
      $select: ['id']
    }) : Promise.resolve();
    ...
    return Promise.props([deviceManifestPromise, applicationTypePromise, parentAppPromise]).then(function(arg1) {
      var applicationTypeId, deviceManifest, extraOptions, parentApplication;
      deviceManifest = arg1[0], applicationTypeId = arg1[1], parentApplication = arg1[2];
  ...
  /**
  	 ...
  	 * });
   */
  exports.generateProvisioningKey = function(nameOrId, callback) {
    return getId(nameOrId).then(function(applicationId) {
      return request.send({
        method: 'POST',
    ...
    })["catch"](noApplicationForKeyResponse, treatAsMissingApplication(nameOrId)).get('body').asCallback(callback);
  };
  ...
  /**
  	 ...
  	 * });
   */
  exports.willTrackNewReleases = function(nameOrId, callback) {
    return exports.get(nameOrId, {
      $select: 'should_track_latest_release'
    }).get('should_track_latest_release').asCallback(callback);
  };
  ...
  /**
  	 ...
  	 * });
   */
  exports.isTrackingLatestRelease = function(nameOrId, callback) {
    return Promise.all([
      ...
      var application, latestRelease;
      application = arg[0], latestRelease = arg[1];
      return application.should_track_latest_release && (!latestRelease || application.commit === latestRelease.commit);
    }).asCallback(callback);
  };
  ...
  /**
  	 ...
  	 * });
   */
  exports.pinToRelease = function(nameOrId, fullReleaseHash, callback) {
    return getId(nameOrId).then(function(applicationId) {
      return pine.patch({
        resource: 'application',
        id: applicationId,
  ...
  /**
  	 ...
  	 * });
   */
  exports.getTargetReleaseHash = function(nameOrId, callback) {
    return exports.get(nameOrId, {
      $select: 'commit'
    }).get('commit').asCallback(callback);
  };
  ...
  /**
  	 ...
  	 * });
   */
  exports.trackLatestRelease = function(nameOrId, callback) {
    return releaseModel().getLatestByApplication(nameOrId, {
      ...
      var applicationId, body, commit;
      applicationId = arg.applicationId, commit = arg.commit;
      body = {
